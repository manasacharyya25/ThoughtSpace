...
export function BlogShareButtons({ url, title }: BlogShareButtonsProps) {
  ...
  return (
    <div className="mt-12 border-t border-[#1C1D1E]/[0.06] pt-8">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {SHARE_OPTIONS.map((option) => (
          <a
            ...
            className={cn(
              "inline-flex items-center gap-2 rounded-xl border border-[#1C1D1E]/10 bg-white px-4 py-2.5 text-xs font-bold text-[#1C1D1E]/70 shadow-[0_4px_12px_rgba(0,0,0,0.03)] transition-colors",
              option.className
            )}
          >
            {option.icon}
            <span className="capitalize">{option.id === "x" ? "X" : option.id}</span>
          </a>
        ))}
      </div>
    </div>
